...
struct PluginCommandTests {
    ...
    private func command(
        _ fixture: Fixture,
        _ action: PluginCommand.Action,
        profileOverride: String? = nil,
        dryRun: Bool = false,
        reloader: any Reloading = StubReloader()
    ) -> PluginCommand {
        PluginCommand(
            action: action,
    ...
    @Test func dryRunToggleWritesNothingAndCreatesNoSnapshot() throws {
        let fixture = try makeFixture()
        try fixture.pointer.set(try ProfileName(validating: "work"))
        let configDir = ConfigDir(url: fixture.activeConf.deletingLastPathComponent().deletingLastPathComponent())
        let before = try SnapshotStore(configDir: configDir).currentSurface()
        var output: [String] = []

        try command(fixture, .enable("theme-sample"), dryRun: true).run { output.append($0) }
        ...
        #expect(fileManager.fileExists(atPath: fixture.activeConf.path) == false)
        #expect(try SnapshotStore(configDir: configDir).currentSurface() == before)
        #expect(SnapshotStore(configDir: configDir).list().isEmpty)
    }
}
